...
package com.demonwav.mcdev.platform.cloud.gradle

import com.demonwav.mcdev.platform.cloud.CloudModuleType
import com.demonwav.mcdev.platform.cloud.creator.CloudRunConfigsStep
...
class CloudRunConfigDataService : AbstractProjectDataService<ProjectData, Project>() {
    ...
    override fun postProcess(
        ...
        val hello = baseDirPath.resolve(Paths.get(".gradle", CloudRunConfigsStep.HELLO))
        ...
        val (moduleName, mcVersion, cloudVersion, task) = lines
        val mcVersionParsed = SemanticVersion.parse(mcVersion)
        val cloudVersionParsed = SemanticVersion.parse(cloudVersion)
        ...
        val isPre113 = mcVersionParsed < CloudModuleType.FG3_MC_VERSION
        if (isPre113 && cloudVersionParsed < CloudModuleType.FG3_CLOUD_VERSION) {
    ...
    private fun cleanupGeneratedRuns(project: Project, module: Module, hasData: Boolean) {
        invokeAndWait {
            if (!module.isDisposed) {
                CloudRunManagerListener(module, hasData)
            }
        }
...
class CloudRunManagerListener(private val module: Module, hasData: Boolean) : RunManagerListener {
