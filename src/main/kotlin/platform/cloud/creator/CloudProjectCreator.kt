...
class Fg2ProjectCreator(
    ...
    private val config: CloudProjectConfig,
    ...
    private fun setupMainClassStep(): BasicJavaClassStep {
        return createJavaClassStep(config.mainClass) { packageName, className ->
            com.demonwav.mcdev.platform.cloud.creator.Fg2Template.applyMainClass(
                project,
                buildSystem,
                config,
                packageName,
    ...
    override fun getSteps(): Iterable<CreatorStep> {
        val buildText = com.demonwav.mcdev.platform.cloud.creator.Fg2Template.applyBuildGradle(
            project,
            buildSystem,
            mcVersion
        )
        val propText = com.demonwav.mcdev.platform.cloud.creator.Fg2Template.applyGradleProp(project, config)
        val settingsText = com.demonwav.mcdev.platform.cloud.creator.Fg2Template.applySettingsGradle(
            project,
        ...
        return listOf(
            ...
            CloudRunConfigsStep(buildSystem, rootDirectory, config, CreatedModuleType.SINGLE)
        )
    }
...
open class Fg3ProjectCreator(
    ...
    protected val config: CloudProjectConfig
) : BaseProjectCreator(rootModule, buildSystem) {
...
class McmodInfoStep(
    ...
    override fun runStep(indicator: ProgressIndicator) {
        val text = com.demonwav.mcdev.platform.cloud.creator.Fg2Template.applyMcmodInfo(project, buildSystem, config)
        val dir = buildSystem.dirsOrError.resourceDirectory
        runWriteTask {
            CreatorStep.writeTextToFile(project, dir, CloudConstants.MCMOD_INFO, text)
        }
    }
}
...
class Fg3ProjectFilesStep(
    ...
    override fun runStep(indicator: ProgressIndicator) {
        ...
        val packDescriptor = CloudPackDescriptor.forMcVersion(config.mcVersion) ?: CloudPackDescriptor.FORMAT_3
        val additionalData = CloudPackAdditionalData.forMcVersion(config.mcVersion)
        val packMcmetaText =
            Fg3Template.applyPackMcmeta(project, buildSystem.artifactId, packDescriptor, additionalData)
        val dir = buildSystem.dirsOrError.resourceDirectory
        runWriteTask {
            CreatorStep.writeTextToFile(project, dir, CloudConstants.PACK_MCMETA, packMcmetaText)
            val meta = dir.resolve("META-INF")
            Files.createDirectories(meta)
            CreatorStep.writeTextToFile(project, meta, CloudConstants.MODS_TOML, modsTomlText)
        }
    }
}
