...
import com.demonwav.mcdev.platform.cloud.version.CloudVersion
...
class CloudProjectSettingsWizard(private val creator: MinecraftProjectCreator) : MinecraftModuleWizardStep() {
...
//    private lateinit var architecturyCheckbox: JCheckBox
    ...
    private lateinit var cloudVersionBox: JComboBox<SemanticVersion>
    ...
    private var config: CloudProjectConfig? = null

    private data class CloudVersions(
        var cloudVersion: CloudVersion,
        var forgeVersion: ForgeVersion,
        var fabricVersion: FabricVersion
    )

    private var versions: CloudVersions? = null

    private var currentJob: Job? = null

    private val cloudVersionBoxListener = ActionListener {
        val selectedVersion = cloudVersionBox.selectedItem as? SemanticVersion ?: return@ActionListener
    ...
    override fun updateStep() {
        val (conf) = modUpdateStep<CloudProjectConfig>(creator, modNameField) ?: return
        config = conf

        title.icon = PlatformAssets.CLOUD_ICON_2X
        title.text = "<html><font size=\"5\">Cloud Settings</font></html>"
    ...
    private fun setCloudVersion(data: Data) {
        // TODO: Increase size and move up
        cloudVersionBox.model = CollectionComboBoxModel(data.cloudVersions.subList(0, min(50, data.cloudVersions.size)))
        cloudVersionBox.selectedIndex = data.cloudSelectedIndex
    }
    ...
    override fun isStepVisible(): Boolean {
        return creator.config is CloudProjectConfig
    }
...
//        setArchitecturyApiVersion(data)
    ...
    private suspend fun downloadVersions() = coroutineScope {
        val cloudVersionJob = async(Dispatchers.IO) { CloudVersion.downloadData() }
...
//        val architecturyApiVersionJob = async(Dispatchers.IO) { CloudVersion.downloadData() }

        versions = CloudVersions(
            cloudVersionJob.await() ?: return@coroutineScope,
...
//            architecturyApiVersionJob.await() ?: return@coroutineScope
    ...
    private suspend fun updateForm(): Data? = coroutineScope {
        ...
        val cloudVersionsJob = async(Dispatchers.IO) { vers.cloudVersion.getCloudVersions(selectedVersion) }
        ...
        val cloudVersions = cloudVersionsJob.await()
...
//        val architecturyApiVersions = architecturyApiVersionsJob.await()
    ...
    private data class Data(
        ...
        val cloudVersions: List<SemanticVersion>,
        val cloudSelectedIndex: Int,
